        ...
        $.jAlert({
            'type': 'confirm', 'confirmQuestion': 'Bạn xác nhận xóa ?', 'onConfirm': function () {
                ...
                setTimeout(function () {
                    ...
                    $.ajax({
                        url: "/Quanlywebsite/DientuMathangAdmin/Deleteone",
                        type: "Post",
                        data: { Id: Id },
                        datatype: "json",
                        success: function (data) {
                            if(data==1)
                            {
                                location.href = "/Quanlywebsite/DientuMathangAdmin/Read";
                            }
                            else
                            {
                                alert_Role();
                ...
                if (id.length > 0) {
                        ...
                        setTimeout(function () {
                            $.ajax({
                                ...
                                url: "/Quanlywebsite/DientuMathangAdmin/DeleteAll",
...
//code xử lý cho xóa thuộc tính

function ClickDeleteTT(IdTag) {
    $.jAlert({
        'type': 'confirm', 'confirmQuestion': 'Bạn xác nhận xóa ?', 'onConfirm': function () {
            //star loading
            $("body").addClass("loading");
            setTimeout(function () {
                //star xử lý hàm ajax ở đây
                $.ajax({
                    url: "/Quanlywebsite/DientuThuoctinhAdmin/Deleteone",
                    type: "Post",
                    data: { IdTag: IdTag },
                    datatype: "json",
                    success: function (data) {
...
//code xử lý cho xóa cấu hình
function ClickDeleteCH(IdTag) {
    $.jAlert({
        'type': 'confirm', 'confirmQuestion': 'Bạn xác nhận xóa ?', 'onConfirm': function () {
            //star loading
            $("body").addClass("loading");
            setTimeout(function () {
                //star xử lý hàm ajax ở đây
                $.ajax({
                    url: "/Quanlywebsite/DientuChitiethinhAdmin/Deleteone",
                    type: "Post",
                    data: { IdTag: IdTag },
                    datatype: "json",
                    success: function (data) {
