$(document).ready(function () {
    ...
    $("#CKdisabled").click(function () {
        if ($("#SelectThuonghieuDt").attr("disabled") == "disabled") {
            $("#SelectThuonghieuDt").removeAttr('disabled');
        }
        else {
            $("#SelectThuonghieuDt").attr('disabled', 'disabled');
        }
        // return false;
    });
    $("#SelectThuonghieuDt").attr('disabled', 'disabled');
...
$("#Insert").click(function () {
    ...
    $("#TxtOrd").val("1");
    $("#CKActive").val("1");
    $('#SelectThuonghieuDt option').removeAttr('selected').filter('[value=""]').attr('selected', true);
    //trượt tới vị trí panner
    $('html, body').animate({
        scrollTop: $("#update_paner").offset().top
    }, 500);
