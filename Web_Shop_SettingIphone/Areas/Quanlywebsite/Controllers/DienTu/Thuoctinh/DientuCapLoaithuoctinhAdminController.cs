...
{
    public class DientuCapLoaithuoctinhAdminController : Controller
    {
        //
        // GET: /Quanlywebsite/DientuCapLoaithuoctinhAdmin/

        private Web_Shop_SettingIphoneEntities connect_entity = new Web_Shop_SettingIphoneEntities();
        DientuCapLoaithuoctinh_Service Connect = new DientuCapLoaithuoctinh_Service();
        int Resurt;
        ...
        {
            var data = connect_entity.DientuCapLoaithuoctinhs.Find(Id);
        ...
        public JsonResult Get_Read(int Id)
        {
            try
            {
                var data = connect_entity.DientuCapLoaithuoctinhs.FirstOrDefault(x => x.Id == Id);
        ...
        [ValidateInput(false)]
        [Authorize(Roles = "Admin,Personnel")]  //Personnel
        public ActionResult Insert(string Name, string Active, string Ord, string Content)
        {
            DientuCapLoaithuoctinh_Model data = new DientuCapLoaithuoctinh_Model();//gọi model data
            data.Name = Name;
            data.Active = Convert.ToBoolean(Active);
            data.Ord = Convert.ToInt32(Ord);
            data.Content = Content;

            if (connect_entity.DientuCapLoaithuoctinhs.FirstOrDefault
        ...
        [ValidateInput(false)]
        [Authorize(Roles = "Admin,Personnel")]  //Personnel
        public ActionResult Update(string Id, string Name, string Active, string Ord, string Content)
        {
            DientuCapLoaithuoctinh_Model data = new DientuCapLoaithuoctinh_Model();//gọi model data
